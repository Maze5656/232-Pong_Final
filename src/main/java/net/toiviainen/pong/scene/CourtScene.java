...
public class CourtScene extends AbstractScene {
	...
	public CourtScene(PongApplication application, boolean mode) throws NullPointerException {
		...
		// if mode is true, game is single player,
		// turn off 2 player controls
		if (mode) {
			// Activate 1 player controls
			setOnKeyPressed(x -> {
				switch (x.getCode()) {
					case W:
